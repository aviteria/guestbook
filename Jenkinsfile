...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="aviteria/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/aviteria/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.host.url=http://192.168.137.143:9000 \
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_aviteria') {
                        oDockImage.push()
                    }
                }
            }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.137.144 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@192.168.137.144 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=192.168.137.140 \
                                        -e MYSQL_PORT=3306 \
